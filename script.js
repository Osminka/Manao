            ...
            success: function (response) {
                ...
                if (jsonData.success == 0) {
                    if (jsonData.message && jsonData.field) {
                        ...
                        document.querySelector('form').insertBefore(errorElement, document.getElementById(jsonData.field).nextSibling)
                    }
                    // $('form')[0].reset();
                }
                else{
...
document.querySelectorAll('input').forEach(element => {
    element.addEventListener('input', (e)=>{
        ...
        if (e.data == ' ') {
            ...
            document.querySelector('#form').insertBefore(errorElement, element.nextSibling)
        }else{
